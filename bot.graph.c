...
#define NONE  -1 /* used in drawing grid */
#define ERROR -1
...
int maxX = 38, maxY = 22;  /* largest values on grid */
...
void printStats()
{
   ...
   printw("Hit 'Enter' or 'Return' to continue..."), getch();
   wattrset(stdscr, COLOR_PAIR(COLOR_BLUE));
   clear(), refresh();
}
...
void printGrid()
{
   ...
   move(1, 1), hline('_', 77); /* draw top line */
   refresh();
